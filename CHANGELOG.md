...
- **Routing keyword coverage expansion** (benchmark-driven): burp suite family, pcap/wireshark, root-detection/certificate-pinning, buffer overflow, `.so`/native/JNI, go binaries (incl. Chinese hints), js-encrypt, webshell, privilege escalation, S3/object storage, memory dump, incident response, Bluetooth/BLE, USB, Unity/game reverse, security assessment, and more.
...
- Routing: sigma vs malware, LLM jailbreak vs iOS jailbreak, full-pentest/domain-compromise vs AD domain control, forensics vs OT ics; master-route.ps1 rewritten UTF-8 BOM for PS 5.1 CJK
