...
# Each tool entry format: name|skill|purpose|version_args|fallback_commands
...
declare -a TOOL_CATALOG=(
    ...
    "pip|reverse-engineering|Python package manager|--version|pip3,pip"
    "node|js-reverse|Run Node-side JS repro and MCP client|--version|node"
    "npx|js-reverse|Run ad-hoc npm packages and MCP entrypoints|--version|npx"
    ...
    "xquik-mcp|threat-intelligence|Remote public X threat-intelligence MCP||"
    ...
    "sstimap|pentest-tools|SSTI auto-detection and exploitation|-h|sstimap"
    ...
    # ─── Classic Kali preinstalled tools previously not listed ───
...
# Parse a tool entry and detect availability
...
# Check whether the MCP server is registered
...
# Get the capability definition from the manifest (requires jq)
