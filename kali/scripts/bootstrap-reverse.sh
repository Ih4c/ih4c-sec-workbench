#!/usr/bin/env bash
# bootstrap-reverse.sh — Kali Linux auto-install / gap-filling tool
...
if [[ ${#CAPABILITIES[@]} -eq 0 ]]; then
    ...
    echo ""
    echo "Examples:"
    echo "  $0 mcp-kali-server metasploitmcp hexstrike-ai pentestswarm  # all pentest MCP servers"
...
install_git_commit() {
    ...
    if [[ -d "$install_dir/.git" ]]; then
        ...
        if [[ -n "$status" ]]; then
            log_err "Existing checkout has local modifications; refusing to proceed: $install_dir"
            return 1
        fi
        return 0
    fi
...
install_github_release() {
    ...
    if [[ -z "$asset" || "$asset" == "null" ]]; then
        log_err "No release asset matched $asset_regex (tag=${release_tag:-latest})"
        return 1
    fi
...
# Register the MCP server in the Claude config
register_mcp_server() {
    local server_name="$1"
    ...
    if command -v jq &>/dev/null; then
        ...
        log_warn "jq is not installed; cannot auto-register the MCP server. Please edit $config_path manually"
    fi
}
...
install_manifest_release() {
    ...
    asset_sha256=$(manifest_field "$capability" assetSha256) || {
        log_err "manifest is missing $capability.assetSha256; refusing to download an unpinned asset"
        return 1
    }
...
ensure_capability() {
    ...
    if command -v "$name" &>/dev/null; then
        log_ok "$name already available: $(command -v "$name")"
        return 0
    fi
    ...
    case "$name" in
        ...
        seclists)
            ...
            ;;
        # ─── New tools in Kali 2026.1 (all installed via apt) ───
        adaptixc2)
        ...
        aircrack-ng)
            ...
            ;;
        # ─── Native Kali MCP tools (apt install + MCP registration) ───
        mcp-kali-server)
            ...
            log_info "Then connect your AI client to the API server via MCP"
            ;;
        metasploitmcp)
        ...
        hexstrike-ai)
            ...
            log_info "HexStrike AI installed. 150+ security tools are exposed to AI agents via MCP."
            ;;
        # ─── Pentest Swarm AI (swarm-intelligence pentest framework) ───
        pentestswarm)
            if command -v pentestswarm &>/dev/null; then
                log_ok "pentestswarm already available"
        ...
        idalib-mcp)
            ...
            log_info "Run ida-pro-mcp --install to finish the IDA plugin installation"
            ;;
        proxycat)
...
EOF
        ...
        jeb-pro)
            ...
            log_warn "JEB Pro is a commercial tool; please obtain a valid license from PNF Software and install it manually. Any community MCP bridge must first be reviewed per skill-supply-chain.md."
        ...
        reqable-mcp)
            ...
            log_warn "Reqable MCP additionally requires the Reqable desktop client with its local API enabled."
            ;;
        jshookmcp)
            if ! command -v node &>/dev/null; then
                install_apt_package "nodejs"
        ...
        xquik-mcp)
            ...
            log_info "Xquik remote MCP registered. Complete OAuth from your MCP client."
            ;;
        agent-browser)
            if ! command -v node &>/dev/null; then
                install_apt_package "nodejs"
        ...
        anything-analyzer)
            ...
            ;;
        idapro)
            # First ensure idalib-mcp is installed
        ...
        burpsuite-mcp)
            ...
            log_warn "Kali ships BurpSuite; search the extension marketplace for the MCP plugin and install it"
...
start_anything_analyzer() {
    ...
    if test_tcp_port 23816 2>/dev/null; then
        log_ok "anything-analyzer already running (port 23816)"
        return 0
    fi
    ...
    if wait_for_port 23816 120; then
        ...
        log_err "anything-analyzer start timed out; see logs: /tmp/anything-analyzer.log"
        return 1
    fi
}
...
start_idapro_service() {
    if test_tcp_port 13337 2>/dev/null; then
        log_ok "IDA Pro MCP already running (port 13337)"
        return 0
    fi
    ...
    if [[ -x "$ida_start_script" ]]; then
        ...
        log_warn "IDA start script not found: $ida_start_script"
        log_warn "Please start IDA Pro manually; the plugin will listen on port 13337"
        return 1
    fi
}
