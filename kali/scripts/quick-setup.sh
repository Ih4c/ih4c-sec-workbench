#!/usr/bin/env bash
# quick-setup.sh — one-shot initialization for Kali 2026.1
# Run this script on a fresh Kali system to automatically:
#   1. Update the system
...
if [[ -f /etc/os-release ]]; then
    ...
    warn "Cannot detect the system version; continuing ..."
fi
...
for tool in "${NEW_TOOLS_2026_1[@]}" "${NEW_TOOLS_2025_4[@]}"; do
    if dpkg -l "$tool" &>/dev/null 2>&1; then
        ...
        apt-get install -y -qq "$tool" 2>/dev/null && ok "$tool installed" || warn "$tool install failed (may not yet be in your sources)"
    fi
done

# ─── Install native Kali MCP ──────────────────────────────────────────────────────────

banner "Configuring native Kali MCP"
...
# ─── Install reverse tools not preinstalled ───────────────────────────────────────────────────────────

banner "Installing reverse engineering tools"

# jadx (not preinstalled on Kali; download from GitHub)
...
if command -v jq &>/dev/null; then
    ...
    warn "jq is not installed; cannot configure MCP automatically. Please copy kali/mcp-kali-example.json manually"
...
echo -e "${BOLD}✅ Kali 2026.1 reverse skill router package configured${RESET}"
echo ""
...
echo "    2. Or simply ask your AI: 'read kali/RULES-kali.md and run the configuration'"
echo "    3. Future security/reverse engineering tasks will be routed automatically"
echo ""
