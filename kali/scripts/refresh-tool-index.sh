...
# Equivalent to the Windows refresh-tool-index.ps1
...

{
    ...
    echo "- Caveat: for MCP servers like jshookmcp, \`yes\` only means this machine can launch it via node/npx — it does not mean it is registered and enabled in the MCP config."
    echo ""
...
if command -v jq &>/dev/null; then
    ...
    # Fall back to minimal JSON when jq is missing
