#!/usr/bin/env bash
# ida-start.sh — start the IDA Pro MCP HTTP service (Linux edition)
# Equivalent to the Windows ida-reverse/scripts/start.ps1

set -euo pipefail

# ─── Configuration (adjust to your actual install) ─────────────────────────────────
...
while [[ $ELAPSED -lt $TIMEOUT ]]; do
    if nc -z 127.0.0.1 "$MCP_PORT" 2>/dev/null; then
        echo "OK: IDA MCP service ready (PID=$MCP_PID, port=$MCP_PORT)"
        exit 0
    fi
    sleep 2
    ELAPSED=$((ELAPSED + 2))
