...
> `<SKILL_ROOT>` is the actual path of this package's `skills/` directory; the AI should detect it automatically.
...
| Python (render script) | ✓ | Already in bootstrap | `scripts/render_diagram.py` dependency |

### Notes

This skill primarily outputs text-based diagram source (Mermaid/DOT/PlantUML) and does not necessarily need a local rendering tool. A renderer is only needed when the user explicitly asks to generate PNG/SVG/PDF files.

If a renderer is unavailable, `scripts/render_diagram.py` prints an installation hint instead of erroring out.
...
**Trigger conditions**: user says "画图", "流程图", "架构图", "攻击路径图", "时序图", "Mermaid", "Graphviz", "PlantUML"
**Downstream exit**:
- Generated diagrams can be embedded in `docs-generator/` reports
- Attack path diagrams can accompany `pentest-tools/` pentest reports

**Peer modules**: `docs-generator/` (embedding diagrams in reports)
