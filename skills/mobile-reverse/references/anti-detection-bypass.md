...
|--------|---------|---------|
| RootBeer | combination of 8 checks | Hook each check method to return false |
...
| Custom native detection | reads /proc/self/status via syscalls | Hook syscalls or modify the /proc mount |
...
// 2. fork detection (forbidden inside sandbox)
...
```javascript
// 1. ptrace on self → prevents attachment
...
Interceptor.attach(fopen, {
    ...
    onLeave: function(retval) {
        if (this.path && this.path.includes("status")) {
            // Modify the returned FILE*, return forged content
        }
    }
});
...
Layer 2 — OkHttp CertificatePinner: Hook to empty the pins list
