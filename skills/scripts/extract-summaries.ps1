...
# Generates skills/INDEX.md: extracts name+description from each module SKILL.md frontmatter and builds the navigation index.
# Idempotent: repeated runs produce identical output (CI validates against drift with git diff).
# Usage:
...
# Scan module SKILL.md files tracked in the repo (excludes local ignored/private trees so a clean clone matches the dev machine).
...
foreach ($sf in $skillFiles) {
    ...
    foreach ($line in $head) {
        ...
        if ($blockMode) {
            # YAML block: join indented text lines (take the first line; truncate if too long)
            if ($line -match '^\s{2,}(.+)$') {
...
[void]$sb.AppendLine('> To change a summary, edit the `description` in the corresponding module `SKILL.md` frontmatter, then rerun the script.')
[void]$sb.AppendLine('')
...
[void]$sb.AppendLine('For ambiguous cases read the full matrix in `skills/routing.md`; CTF multi-type tasks go to `CTF-Sandbox-Orchestrator/`.')

$newContent = $sb.ToString()
# Normalize to LF line endings: consistent with .gitattributes (*.md eol=lf) so -Check stays idempotent after clone
