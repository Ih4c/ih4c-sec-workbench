...
# --- New source-of-truth / artifact checks (routing.json / benchmark / INDEX) ---
...
if (Test-Path -LiteralPath $benchJson) {
    ...
    # benchmark expect ids must exist in routing.json (prevents benchmark referencing deleted routes)
    if (Test-Path -LiteralPath $routingJson) {
...
foreach ($rp in @($rulesEn)) {
    ...
    # Authorization contract: assume-authorized per precedent-auth (the fork's model)
    if ($rt -match 'precedent-auth' -and $rt -match 'assume.?authorized|authorization assumed') {
        Ok "$name has assume-authorized contract"
    } else {
        Bad "$name missing assume-authorized contract"
    }
...
Assert-Fields (Join-Path $skillsRoot 'reverse-engineering/references/re-agent-workflow.md') @('Triage', 'Static', 'Dynamic', 'Synthesis', 'IAT repair iron rule', 'E-iat-repair-fail', 'E-exports', 'dnSpy', 'feasibility gate', 'E-self-check-crash', 'ExitProcess', 'time box', 'E-api-hash', 'E-anti-debug-peb', 'E-wide-strings', 'A–T', 'U–AV', 'nonpe-format-cookbook')
Assert-Fields (Join-Path $skillsRoot 'pentest-tools/references/recon-pipeline.md') @('precedent-auth', 'network_profile', 'Evidence', 'nuclei')
...
Assert-Fields (Join-Path $skillsRoot 'docs-generator/SKILL.md') @('vendor-report-rules.md', 'flavor = null', 'no forced IOC/ATT&CK')
...
if ($vendorRulesText -match 'first confirm scope and preserve evidence' -and $vendorRulesText -match 'MUST NOT delete files directly before evidence preservation') {
...
if ($vendorRulesText -match '(?m)JS/Web signature reverse\s*\|[^\r\n]*`malware`') {
...
if ($vendorRulesText -match '(?m)vuln.*default full-text' -or $vendorRulesText -match 'third default full-text flavor') {
    # presence of explicit "not third default" language is OK; flag only if it claims vuln IS a third default full flavor
}
if ($vendorRulesText -match 'only 2 full-text vendor flavors' -or $vendorRulesText -match 'not.*third default full-text flavor') {
...
# Unified rule: exactly one of pinnedVersion / pinnedCommit / pinPolicy must be set;
# github-release-* additionally accepts assetSha256 / preferApiDigest (GitHub official release asset hashes).
# local-http-mcp may skip the pin only when it does not fetch external source code.
...
foreach ($mf in @($skillsManifest, $kaliManifest)) {
    ...
    foreach ($cap in $mc.capabilities) {
        ...
        switch ($capMap['bootstrapKind']) {
            ...
            'winget-package' { $hasPin = $hasPin } # winget-latest falls under pinPolicy
            'apt-package' { $hasPin = $true }      # shipped by distro repository (Kali side)
            'docker-image' { $hasPin = $true }     # fallback channel
            'manual' { $hasPin = $true }           # manual install
...
if ($contribText -like "*genuine decision boundary*" -and $contribText -notlike "*every phase ends with 3-6 numbered options*") { Ok "new-skill contract uses genuine decision boundaries" } else { Bad "new-skill contract still requires per-stage menus" }
...
if ($reWorkflowText -like "*decision_delta*" -and $reWorkflowText -like "*carry_forward_refs*" -and $reWorkflowText -like "*consumer must first inherit the refs*") { Ok "representative RE workflow consumes delta by reference" } else { Bad "representative RE workflow missing delta consumer contract" }
