...
# Rule single source of truth: skills/config/routing.json (do NOT hardcode a routing table in this script).
# CLI backward compatible: -Hint / -OutDir; writes route-scope.md; exit code 0 success / 2 config or skill missing.
# Read UTF-8 BOM sources so CJK renders correctly under Windows PowerShell 5.1.
param(
    [string] $Hint = '',
    [string] $OutDir = '',
    [string] $ProjectRoot = ''
...
# --- Load routing config (single source of truth) ---
...
# --- Rule matching: every keyword rule that hits is added to the candidate set (consistent with the old per-rule if behavior) ---
...
foreach ($route in $cfg.routes.PSObject.Properties) {
    ...
    foreach ($kw in $route.Value.keywords) {
        ...
        # mustAll: all sub-regexes must match
        if ($hit -and $null -ne $kw.mustAll) {
            foreach ($m in $kw.mustAll) {
                if ($t -notmatch $m) { $hit = $false; break }
            }
        }
        # exclude: if it hits, this rule does not fire (prevents false positives, e.g. jailbreak/LLM contexts, domain-controller/attack-chain cases)
        if ($hit -and $null -ne $kw.exclude -and $t -match $kw.exclude) { $hit = $false }
        if ($hit) { [void]$sel.Add($id) }
    }
}

# --- Scoring: multiple rule hits for the same id add points (consistent with old behavior) ---
$scores = [ordered]@{}
foreach ($item in $sel) {
...
# --- priority self-check: routes and priority must correspond 1:1 so a new route cannot be added without a priority ---
...
# --- Pick the highest-scoring route in priority order as PRIMARY (ties resolved by earlier priority) ---
...
# Guard: if priority contains a ghost id outside routes (config error), fall back to fallbackId instead of crashing
...
# Reuse the current mainline project work-root resolution so route artifacts are not written into the skill package.
$scriptDir = $PSScriptRoot
if (-not $scriptDir) { $scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path }
$skillsRoot = Split-Path -Parent $scriptDir
$packageRoot = Split-Path -Parent $skillsRoot
...
# --- Write route-scope.md (format consistent with old version; downstream scripts parse primary_skill / primary) ---
