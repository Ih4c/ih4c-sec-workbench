...
# reverse-skill routing regression tester: runs every routing-benchmark.json case and compares the expected PRIMARY.
# Usage:
...
# Exit codes: 0 all green / 1 one or more failed cases (failure details written to LogDir)
param(
    [string] $Benchmark = '',
    [switch] $Quick,
    [string] $LogDir = '',
...
# $env:TEMP may not be set on Linux/macOS runners; uniformly fall back to GetTempPath
