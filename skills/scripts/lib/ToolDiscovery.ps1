...
function Get-ReverseToolCatalog {
    ...
    return @(
        ...
        [pscustomobject]@{
            Name = 'java'
            Skill = 'apk-reverse'
            Purpose = 'Runs jar and Java toolchain'
            VersionArgs = @('-version')
            Fallbacks = @(
        ...
        [pscustomobject]@{
            Name = 'node'
            Skill = 'js-reverse'
            Purpose = 'Runs Node-side JS reproduction and MCP client'
            FixedVersion = 'v0.5.0'
            VersionArgs = @()
            Fallbacks = @(
        ...
        [pscustomobject]@{
            Name = 'npx'
            Skill = 'js-reverse'
            Purpose = 'Runs ad-hoc npm packages and MCP entrypoints'
            FixedVersion = 'v0.5.0'
            VersionArgs = @()
            Fallbacks = @(
        ...
        [pscustomobject]@{
            Name = 'jshookmcp'
            Skill = 'js-reverse'
            Purpose = 'Starts @jshookmcp/jshook MCP via npx (requires MCP registration; npx alone does not mean this capability is installed)'
        ...
        [pscustomobject]@{
            Name = 'reqable-mcp'
            Skill = 'pentest-tools'
            Purpose = 'Starts Reqable desktop client MCP via npx (requires MCP registration and Reqable; npx alone does not mean this capability is installed)'
        ...
        [pscustomobject]@{
            ...
            Purpose = 'Remote MCP for public X/Twitter threat intelligence collection (requires client registration and OAuth)'
            VersionArgs = @()
            Fallbacks = @()
        }
        [pscustomobject]@{
        ...
        [pscustomobject]@{
            Name = 'jeb-pro'
            Skill = 'apk-reverse'
            Purpose = 'JEB Pro commercial Android / ARM decompiler (user must provide a valid license)'
            VersionArgs = @()
            Fallbacks = @(
        ...
        [pscustomobject]@{
            Name = 'pentestswarm'
            Skill = 'pentest-tools'
            Purpose = 'Swarm-intelligence autonomous pentest and MCP execution'
            FixedVersion = 'v0.5.0'
            VersionArgs = @()
            Fallbacks = @(
