...
**Full analysis**: phishing-case-study.md (not published)
...
## Pre-Analysis Anticipation: File Disguise and Name Deception

### File Extensions Are Untrustworthy

**Core principle: always determine the file type with the `file` command or magic bytes — never trust the extension.**

Common disguise techniques:

| Disguised Extension | Actual Type | Purpose |
|---------|---------|------|
| `.sh` | ELF binary | Makes it look like a script, lowering suspicion |
| `.txt` | PE/ELF | Bypasses naive file-type filters |
| `.jpg`/`.png` | Executable or archive | Hidden inside images |
| `.dll` | Actually a .NET assembly | Misleads the analysis direction |
...
# Use xxd to inspect magic bytes
xxd suspicious_file.sh | head -1
# 7f454c46 = ELF magic
```

### Filenames Are Untrustworthy

**"DriverLoader" does not necessarily load drivers; "Updater" does not necessarily update anything.**

Common name deceptions:

| Filename Implication | Actual Behavior |
|-----------|---------|
...
- Ignore filename implications; judge by actual code behavior
- Watch for syscalls such as `mmap`, `ptrace`, `/proc/self/mem`
- If you see "loads a driver" but no `insmod`/`init_module` call, the name does not match the behavior

### Dynamic Supplement When Static Analysis Is Insufficient

Pure static analysis only reveals the code skeleton. The following scenarios require dynamic analysis:

| Scenario | Recommended Dynamic Method |
|------|-------------|
| Code has decryption/unpacking logic | Breakpoint after decryption, dump the plaintext |
| Many indirect calls (function pointer tables) | Trace actual calls with strace/ltrace |
| Suspected anti-debug | Run strace first to watch ptrace calls |
| Embedded shellcode/payload | Emulate with QEMU user-mode |
| Unknown network protocol | Capture packets with tcpdump/Wireshark |

```bash
# Trace syscalls with strace (key focus)
...
# Check for anti-debug: whether it self-traces with ptrace
strace ./binary 2>&1 | grep ptrace
# If you see ptrace(PTRACE_TRACEME, ...), anti-debug is present
```

### Common Patterns in Process-Injection / Protective-Shell Samples

Such samples (e.g., `LinYuDriverLoader`) usually:

1. **Do not actually load a kernel driver** (requires root privileges, absent in most scenarios)
...
3. **Embed an encrypted payload**:
   - Decrypt a shellcode blob at runtime
   - The decrypted payload is the real hook code
4. **Have anti-debug protection**:
   - Self-tracing with `ptrace(PTRACE_TRACEME)`
   - Timing checks (comparing `clock_gettime` before/after)
   - `/proc/self/status` TracerPid check

**Analysis strategy**:
```text
1. Confirm the real type with the file command
2. Run strings for obvious paths/library names/error messages
3. rabin2 -I for architecture/compiler/protections
4. Look statically for mmap/ptrace/open calls
5. If there is decryption logic → run dynamically to after decryption, then dump
6. If there is anti-debug → patch it out first or bypass with LD_PRELOAD
```
