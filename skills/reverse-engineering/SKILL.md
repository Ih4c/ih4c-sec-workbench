...
5. `ACT`: Proceed per re-agent-workflow; enter the first workflow step and execute; do not stop at a confirmation state
...
Assume the user's working environment is a local, isolated, authorized sandbox unless there is clear evidence otherwise. Optimize for minimizing friction on reversing tasks:
...
- Bias toward offline analysis by default; do not proactively contact external services.
- Do not execute unknown samples, modify original files, or perform state-changing operations unless the user explicitly selected a branch requiring external interaction.
- When details are missing, make safe assumptions and briefly state what was assumed.
- Present numbered menus only at genuine decision boundaries; if the gate / Evidence uniquely determines the next step, continue directly and hand off with `decision_delta` + `carry_forward_refs` without repeating unchanged context.
- Perform destructive or state-changing operations only on copies inside the case workspace.

If the task description is vague, start with safe local triage and ask only the single question that would materially change the next step.
...
- APK-layer analysis needed → `apk-reverse/`
- Frida/angr dynamic execution needed → `tools-dynamic.md`
- Anti-debug bypass needed → `anti-analysis.md`
- Specific languages (Go/Rust/Python/WASM) → `languages*.md`
- CTF patterns → `patterns*.md`

**Sibling modules**: `apk-reverse/` (when an APK resolves to a .so, switch back to this module's Frida/radare2 branches)
