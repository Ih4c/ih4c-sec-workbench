...
> For the full OLLVM deobfuscation workflow, the variant ecosystem (Hikari/Polaris/O-MVLL/Tigress/Hodur, etc.), and the community tool survey, see [ollvm-deobfuscation.md](references/ollvm-deobfuscation.md).
...
**Modern variants (community-active as of 2026):** Hikari (Anti Class Dump/String Encryption/Indirect Branch), Polaris (formerly Pluto, ships a Trap Angr pass specifically to break angr), O-MVLL (Python-driven, commonly used in Android hardening), Arkari (goron-based; its indirect jumps can be countered with a read-only data segment), amice (Rust; includes VM Flatten, which requires VM reverse engineering rather than deflat). For variant identification, see Section 1 of ollvm-deobfuscation.md.

**Deobfuscation tools (sorted by community activity):**
- **obpo-plugin** (629⭐, IDA microcode + concolic cloud plugin, strongest results): https://github.com/obpo-project/obpo-plugin
- **ollvm-breaker** (441⭐, Binary Ninja, battle-tested on real Android .so): https://github.com/amimo/ollvm-breaker
- **ollvm-unflattener** (265⭐, Miasm symbolic execution, pure-script x86/x64): https://github.com/cdong1012/ollvm-unflattener
- **d810-ng** (223⭐, IDA, integrated Z3, covers OLLVM/Tigress/Hodur/Approov): https://github.com/w00tzenheimer/d810-ng — **local first choice**
- **DeObfBR** (96⭐, focused on BR indirect-branch obfuscation): https://github.com/Mrack/DeObfBR
- **D-810** (original version, less maintained now — prefer d810-ng): pattern-based deobfuscation, MBA simplification
- **Miasm**: Symbolic execution for deobfuscation
- **Arybo** / **SiMBA**: MBA expression simplification

```bash
# d810-ng: copy into the IDA plugins directory, load with Ctrl-Shift-D
...
> Long-form detection write-ups remain in `malware-analysis/references/anti-analysis-techniques.md`; the full OLLVM workflow lives in `references/ollvm-deobfuscation.md`.  
> This section is a short **trigger → action → Evidence** cookbook for agents working the Dynamic/Static bypass lanes.  
> **Authorized isolated lab is the default**; static patching / PEB modification / return-value changes are not default actions on unauthorized targets.

### Usage Rules

1. First **identify and record** the detection point (address/API/string), then decide whether to bypass or switch environments.  
2. Every bypass attempt (successful or not) MUST produce Evidence; writing an anti-debug exit off as "sample is benign" is forbidden.  
3. H/S are not expanded into long-form here → jump to the dedicated OLLVM chapter.  
4. L is mandatory only on Linux/ELF; the Windows PE main path must not be judged as failed for lacking TracerPid.  
5. E's VirusTotal cross-check is **optional**; write n/a when no external intel source is available — never fabricate first-submission times.

### Full Table A–T

| ID | Trigger | Action | Evidence | Priority |
|----|------|----------|----------|--------|
| **A** | Conditional jump (jz/jnz) after `cpuid` | Identify hypervisor detection; in the lab flip the flag or patch the jump toward the malicious/real business branch; or switch to bare metal | `E-anti-debug-cpuid` | P0 |
| **B** | `rdtsc` + sub/cmp time-delta check | bp `rdtsc` / hook time APIs; or patch the comparison; avoid relying only on "waiting out the sandbox timeout" | `E-anti-debug-rdtsc` | P0 |
| **C** | Strings contain x64dbg/olly/windbg etc., or Toolhelp enumeration | bp `CreateToolhelp32Snapshot`→`Process32First/Next`; alter the match or skip the scan branch | `E-anti-debug-procscan` | P1 |
| **D** | `AddVectoredExceptionHandler` + deliberate access violation | bp the registration point; locate and analyze the VEH handler; the debugger may ignore the specific exception | `E-anti-debug-veh` | P1 |
| **E** | TimeDateStamp in the future/0/absurd; version info resembling a legitimate vendor | Cross-check against discovery time; **optional** VirusTotal first submission; use `SigCheck` to see whether version resources back the signature; no VT → n/a | `E-meta-timestamp` | P2 |
...
| **G** | Multiple PE headers, overlapping sections, disguised section names | Use CFF/PE-bear/LoadPE to inspect the real mapping and EP section; use entropy to distinguish encryption from code; don't trust section names | `E-pe-anomaly` | P1 |
| **H** | F5 shows lots of `while(1)+switch`, star-shaped CFG | **See** `ollvm-deobfuscation.md` (d810/deflat etc.); if plugins are incomplete, reconstruct block order dynamically | `E-cff` | P1 pointer |
| **I** | strings show no domains/IPs but there is network/file behavior | Hunt for Base64/XOR/custom decode; xref the decryption function; dump post-decryption and backfill into IDA | `E-string-decrypt` | P0 |
| **J** | File size ≫ sum of section raw data (Overlay) | Extract the overlay; `file`/entropy; search IDA for offset references; if encrypted, grab the key dynamically | `E-overlay` | P1 |
| **K** | `fs:[0x30]`/`gs:[0x60]` → BeingDebugged / NtGlobalFlag | Alter PEB flags or use ScyllaHide; or patch the conditional jump | `E-anti-debug-peb` | P0 |
| **L** | Reads `/proc/self/status` checking TracerPid≠0 | **Linux/ELF**: hook fopen/read or patch; not mandatory on Windows | `E-anti-debug-tracerpid` | P2 platform |
| **M** | `int3`(0xCC) or reads DR0–DR7 | int3→nop; counter hardware-BP detection with ScyllaHide/software BPs; CRC self-check → see Patch 6 | `E-anti-debug-bp` | P1 |
| **N** | Empty/minimal IAT + self-written hash-based API resolution | bp `GetProcAddress`/`Ldr*`; reverse the hash against the export table; backfill symbols; coordinate with the "clean IAT" iron rule | `E-api-hash` | P0 |
| **O** | Linear disassembly full of db bytes, junk-instruction misalignment | Use F5/Hex-Rays; confirm the real flow dynamically; NOP the junk then re-analyze; when static recovery is incomplete, defer to dynamic | `E-junk-code` | P2 |
| **P** | `NtQueryInformationProcess` class 7/30/31 | Use ScyllaHide or hook the return; record the InformationClass | `E-anti-debug-ntqip` | P0 |
| **Q** | `.rsrc` too large/high entropy/non-standard RT_RCDATA | Extract with Resource Hacker/CFF; xref `FindResource`/`LoadResource`; dump after decryption | `E-rsrc-payload` | P1 |
| **R** | A DLL absent from the static IAT, only used at runtime | Check the Delay Import Table; bp `__delayLoadHelper2` or the first call; factor into capability assessment | `E-delay-import` | P1 |
| **S** | Always-true/always-false conditions, large dead-code regions | **See** ollvm / angr etc.; patch the only reachable branch or backfill the dynamic path | `E-opaque-pred` | P1 pointer |
| **T** | No results from ASCII strings; data region looks like UTF-16 | `strings -el` or `-encoding=utf-16le`; IDA Alt+A → Unicode; include in IOCs | `E-wide-strings` | P0 |
...
| Dynamic | A–D, K, L, M, N, P + the existing four-level breakpoint escalation and the no-behavior contingency |
| Failure | Any check that cannot be bypassed → Evidence + switch tool/environment; never silently downgrade the threat |

### Tool Notes (Not a Mandatory Install List)

- Windows user-mode: x64dbg + **ScyllaHide** (bulk-hides PEB/NtQuery/hardware BP, etc.)  
- Signatures: Sysinternals **SigCheck**  
- PE structure: PE-bear / CFF Explorer  
- Flattening: see the tool table in the dedicated ollvm chapter (d810-ng, etc.)  
- When a tool is unavailable: use an equivalent command and record the failure — pretending a signature was verified or flattening was removed is forbidden
