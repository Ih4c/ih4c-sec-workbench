...
.NET managed programs (C#-compiled exe/dll) have a complete, separate workflow — see [`../dotnet-reverse/SKILL.md`](../dotnet-reverse/SKILL.md).

**Quick reference:**
- Recognition: PE header CLR runtime header non-zero, `mscoree.dll` import, `mscorlib`/`System.Private.CoreLib` strings
- Tools: **dnSpyEx** (decompilation + debugging + IL editing, Windows GUI) / **ILSpy** (CLI) / **de4dot** (deobfuscation) / **dnlib** (scriptable)
...
- **IL first**: make key decisions and patches with an IL editor rather than a C# decompiler (avoids distortion from recompiling compiler-generated code)
- Red-team Sharp* tool analysis (Rubeus/SharpHound) → [`../dotnet-reverse/references/sharp-tools.md`](../dotnet-reverse/references/sharp-tools.md)

**Boundary:** IL2CPP / NativeAOT builds are native (no CLR) — do not use dnSpy; use `ida-reverse/` / `radare2/` instead (see [seed-014](../field-journal/seed-014_unity-il2cpp-reverse.md)). The Codegate 2013 two-stage XOR+AES-CBC pattern is under the `.NET Analysis` section of [tools.md](tools.md#net-analysis).
