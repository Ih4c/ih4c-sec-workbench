...
2. `NOW`: Run `case-init` until `scope.md` is ready; for offline samples use `offline` / `lab`
3. `ACT`: Classify the file starting from Phase 1 of "3. General Reverse Engineering Workflow"; do not stop at the table of contents
...
- [2. DSL VM Identifying Features](#2-dsl-vm-identifying-features)
...
| # | Feature | Notes |
|---|------|------|
| 1 | Starts with an IIFE + many single-letter variable names | `!function(){var U=void 0,y=parseInt,E0=Function,...}` |
...
| 5 | Single-line minified code | 583KB single line, obfuscated variable names |
...
| File contains the WASM magic via `Uint8Array([0,97,115,109])` | Embedded WASM | Extract the .wasm, then use IDA/Ghidra |
...
## 2. DSL VM Identifying Features

### Code Features

```javascript
// Feature 1: IIFE entry; single-letter variables map numeric constants
!function(){
    var U=void 0, y=parseInt, E0=Function, AN=Uint8Array;
    var E=15, l=10, m=12, x=16, S=13, $=11;
    // Numeric constants are mapped to variable names, replacing the raw numbers
    ...
}

// Feature 2: interpreter main loop DG()
function DG(C, d, ...) {
    var d = [];  // Array emulates the WASM stack/locals
    for (d[7] = x; d[7] !== U;) {
...
// All built-in functions are invoked through the C[index] index
...
# 1. Check for the WASM magic bytes
if head[:4] == b'\x00asm':
...
# Analyze the context around each reference
...
Exported functions (e.g., `getToken`) are located through the following path:

```
1. Find the AWSCInner.register() or similar registration call
2. Identify the registered module and its factory function
3. Find the object returned by the factory function → where the export function is defined
...
### Reference Opcode Table (based on prior cases)
...
| 21 | **STORE** | Safely fetch global/object properties |
| 22 | **STRING** | `new fh(r,v)` string/regex handling |
| 23 | **BRANCH** | Safe fetch via `try...catch` + conditional jump |
...
### Approach C: Pure Protocol Validation (extremely low success rate)

> Tokens generated by the DSL VM are usually tightly bound to the browser context (TLS JA3 fingerprint, IP, Cookie, request headers, etc.); once outside the browser, the server can detect the context mismatch. **A pure-protocol approach is not recommended**.
...
| 8776 | **Operation too fast, retry required** | Retry after adding a delay |
...
- [ ] Have I completed DSL VM identification (IIFE + single-letter variables + the DG() interpreter)?
- [ ] Have I extracted the variable mapping table (`var X=<number>`)?
- [ ] Have I extracted the opcode list and classified it?
- [ ] Have I analyzed the reference range of the C[9] constant table?
- [ ] Have I located the export function registration point?
- [ ] When pure static analysis was insufficient, did I try the runtime injection approach?
- [ ] Did I write back to the field-journal after completing the task?
- [ ] Found new tools/new scenarios → update routing.md?
