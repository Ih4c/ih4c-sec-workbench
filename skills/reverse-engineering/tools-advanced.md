...
> For the full OLLVM deobfuscation workflow, the variant ecosystem, and the community tool survey, see [ollvm-deobfuscation.md](references/ollvm-deobfuscation.md).

### d810-ng (IDA) — Local First Choice

The modern maintained version (Next Generation) of D-810, integrating the **Z3 SMT** solver and covering multiple variants: OLLVM / Tigress / Hodur(PlugX) / Approov.
...
- MBA patterns specific to the PlugX (Hodur) malware
...
Built on Hex-Rays **microcode** + data-flow tracking + program slicing + concolic execution. Community consensus ranks it among the strongest.

```text
- Optimizes decompiler output at the microcode level (not by rewriting ASM)
- Supports IDA 7.5-7.7 + Hex-Rays, multiple architectures (ARM/ARM64/x86/x64/PPC/MIPS)
- Cloud plugin: the target function is uploaded to the obpo-server for processing (core closed-source; plugin free)
- ⚠️ Be careful with sensitive samples (the binary is uploaded to a cloud service)
...
No IDA dependency; based on Miasm symbolic execution with BFS multi-layer processing, supporting x86/x64 Windows/Linux.
...
Binary Ninja deflattening; the repo ships the hardened Android sample `libvdog.so` as hands-on practice. Source: amimo/ollvm-breaker

### DeObfBR — Dedicated to BR Obfuscation

Specially removes Goron/Arkari-style BR (indirect-branch) obfuscation. Easy trick: setting the data segment read-only partially counters it. Source: Mrack/DeObfBR

### deollvm — ARM64 Unicorn

An alternative for ARM64 .so when IDA is unavailable, based on Unicorn. Source: GeT1t/deollvm
