...
3. `NOW`: case-init; record the installer source and test accounts into scope (scope.md is case tracking + network profile; authorization per `../field-journal/precedent-auth.md`)
4. `NEXT`: Tools (Burp upstream proxy, process monitoring, reverse tools)
...
- Auto-update channel and code-signature verification
...
□ API privilege abuse: hidden management interfaces in the client
```
...
- [ ] Was a trust boundary map drawn?
- [ ] Were both the local and network surfaces covered?
- [ ] Checklist?
